...
class Db_Client: 
    ...
    async def read_currentSpeed(self, unit) :
        '''
        Осуществляет чтение метки из базы данных
    ...
    async def read_currentSensor(self) :
        '''
        Осуществляет чтение метки из базы данных
    ...
    async def write_label_lfm(self, name_measurement, label_lfm, freq_value):
        ...
        p = Point(name_measurement).tag("SN", label_lfm.getSerial()).tag("FN", label_lfm.getOpername()).tag(
            ...
            ).tag("Start", label_lfm.getStart()).field("f", freq_value)
    ...
    async def write_label_vfd(self, label_vfd):
        ...
        freq_value = float(label_vfd.getVelocityFreq())       
        
        #client = await self.connect()
        
        p = Point("test-vfd").tag("SN", label_vfd.getSerial()).tag("FN", label_vfd.getOpername()).tag(
            "V", label_vfd.getVelocity()).tag("UNIT", label_vfd.getUnit()).tag("VO", label_vfd.getVo()
            ).tag("Start", label_vfd.getStart()).field("f", freq_value)
        
        
        #tag("SN", label_vfd.getSerial()).tag("FN", label_vfd.getOpername()).tag("V", label_vfd.getVelocity())
