...
class Db_Client: 
    ...
    async def read_currentSpeed(self, unit, sn) :
        ...
        if (unit == "Hz"):
            output = self.read_currentSensor(sn)
            return output
        ...
        #print(output)
        tables = clientS.query_api().query(query=queryT)

        output = tables[0].records[0]["_time"]
        clientS.close()
        print(output)
        '''
        queryT = f'from(bucket: "'+self.bucket+'")' \
    ...
    async def read_currentSensor(self, sn) :
        ...
        freq = tables[0].records[0]["_value"]        
