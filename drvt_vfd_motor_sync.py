...
#import drivers.crc_vfd_ascii
#import crc_vfd_ascii
import lotok.drivers.crc_vfd_ascii
...
class Vfd_Motor_Driver: 
    ...
    def __init__(self, serial_port_vfd_=serial_port_vfd, name_port_=name_port): 
        self.serial_port_vfd = serial_port_vfd_
        self.name_port = name_port_

        try:
            self.serial_port_vfd = serial.Serial(
            port=self.name_port,
            baudrate=9600,
            ...
            print('Serial_port_VFD', self.name_port, 'No Open')
        else:
            print('Serial_port_VFD', self.name_port, 'Open')
    ...
    def SendCommand06(self,addr,data):
        ...
        #TODO - commented fpr testing only!
        cs = lotok.drivers.crc_vfd_ascii.lrc(buffer)
        buffer+= cs + b'\r\n'
    ...
    def exit(self):
        ...
        for n in range(100):
            f = 1.0 + 0.05*n
            V = '%.2f' %f
            #змінити, коли буде ясно, як вичисляти VF
            VF = V
