...
class Reader_Async: 
    ...
    ''' 
    def __init__(self, zmq_sub_, dataclass_reader_async_):
        
        
        self.zmq_sub = zmq_sub_
        self.name_measurement = dataclass_reader_async_.getName_measurement()
        self.port = dataclass_reader_async_.getPort()
        self.baudrate = dataclass_reader_async_.getBaudrate()
        self.divide_coef = dataclass_reader_async_.getDivide_coef()
        self.bytesize = dataclass_reader_async_.getBytesize()
        self.parity = dataclass_reader_async_.getParity()
        self.stopbits = dataclass_reader_async_.getStopbits()
        #timeout=0, #0.05
        self.write_timeout = 0
        self.xonxoff = dataclass_reader_async_.getXonxoff()
        self.rtscts = dataclass_reader_async_.getRtscts()
        self.dsrdtr = dataclass_reader_async_.getDsrdtr()
        ...
        while True:
            ...
            if flag == b'/':
                ...
                if (self.zmq_sub.canWriteToDB == True):
                    ...
                    await self.writeToDB(db_client, self.name_measurement, self.zmq_sub.getLastSubReadLabel_lfm(), self.getFrequency())
    ...
    async def writeToDB(self, db_client, name_measurement, label_lfm, freq_value):
        '''
