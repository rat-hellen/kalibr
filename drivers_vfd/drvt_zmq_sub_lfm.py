...
class Zmq_Sub: 
    ...
    def decode_json_message_toLabelLowFreqMeter(self, message):  
        ...
        '''
        #b'{"SN": "test", "FN": "test", "V": "0.3", "UNIT": "Hz", "VO": "False", "Start":start_time }'
        ...
        index = substring.find("VO")
        #print('index =', index,'$')

        if (index!=-1):
            #vo = substring[index+1:substring.find(",")-1]
            vo = substring[index+6:substring.find("Start")-4]
            substring = substring[substring.find(",")+1:]
            #print('!!&&&%!%&&&!!!!!vo =', vo,'$')
        ...
        label = Label_LowFreqMeter(serial,opername,velocity,unit,vo,start)
        return label
    ...
    def getLastSubReadLabel_lfm(self):
        return self.lastSubReadLabel_lfm
